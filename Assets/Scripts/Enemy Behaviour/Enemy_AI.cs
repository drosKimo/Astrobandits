...
public class Enemy_AI : MonoBehaviour
{
    ...
    public IEnumerator EnemyTurn()
    {
        ...
        yield return new WaitUntil(() => turnEnd); // ждет пока ход закончитс€
    ...
    IEnumerator EnemyPlayCard()
    {
        ...
        restart:
        // чтобы можно было добавл€ть карты, нужно поместить значени€ в .List()
            ...
            switch (card.itemName)
            {
                ...
                case "Cards.Name.Shredder":
                    ...
                    if (target.gameObject.tag == "Player")
                    {
                        ...
                        Destroy(playerHand.transform.GetChild(ind).gameObject); // удал€ет ту же карту на экране
                    }
                    else
                    {
    ...
    // удал€ет разыгранные карты
    void DestroyCards()
    {
        // удал€ет использованные карты из руки в обратном пор€дке
        for (int i = index.Count; i > 0; i--)
        {
            characterRole = gameObject.GetComponent<CharacterRole>();
            characterRole.hand.RemoveAt(index[i - 1]);
        }
        index.Clear(); // очищает отбойник после удалени€ карт
    }
}
